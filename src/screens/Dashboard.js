...
import { IoIosArrowDropright, IoMdAdd, IoMdArrowBack, IoMdClose, IoMdExit, IoMdSend } from "react-icons/io";
                         ...
                         ${toUser != "" ? "hidden" : "w-full"}`}
                              >
                                    ...
                                    >
                                          ...
                                          >
                                                <div className="pl-3 text-sm text-white">{user?.name || user?.mobile}</div>
                                                <div className="pl-3 text-xs text-gray-400">{user?.mobile}</div>
                                          </div>
                                    </div>
                                          ...
                                          {userList?.length > 0 ? (
                                                userList.map((item, index) => {
                                                      ...
                                                      rounded-2xl text-white hover:text-black"
                                                            >
                                                                  <img src="logo192.png" className={`w-8 object-contain  rounded-full p-1`} />
                                                                  <div className="pl-3">
                                                                        <div className="text-sm  ">{item?.name || item?.mobile}</div>
                                                                        <div className={`text-xs text-gray-400 ${item?.new ? "font-extrabold" : ""}`}>{item?.message}</div>
                                                                  </div>
                                                                  <div className="flex-1 flex justify-end  text-xs ">{item?.new && <div className=" p-0.5 h-5 w-5 border-green-500 border rounded-full items-center text-center">{item?.new}</div>}</div>
                                                            </div>
                                                      );
                                                })
                                          ) : (
                                    ...
                                    <div onClick={() => setNewContactModal(true)} className="static p-1.5 m-3 bg-teal-600 shadow-sm shadow-gray-600 self-end rounded-full">
                    ...
                    >
                            ...
                            <div className="text-3xl text-white"> Chattify</div>
                              ...
                              <div className={`w-full flex  flex-col m-1 sm:w-[70%] bg-[#000000]  rounded-3xl ${toUser != "" ? "" : "hidden"}`}>
                                          ...
                                          <div className=" flex flex-1   flex-col text-white p-3 text-sm justify-end">
                                                {chatHistory.concat(extendHistory).map((item, index) => {
                                                      return item?.toUser_id == user?.user_id ? (
                                                            ...
                                                            <div className="self-end duration-300 transition-all  bg-green-900 bg-opacity-80 px-2 py-1 rounded-l-lg rounded-b-lg my-1"> {item?.message}</div>
                                                      );
                                                })}
                                                <div
                                                      ref={scrollRef}
                                    ...
                                    >
                                          <input value={message} onChange={(val) => setMessage(val.target.value)} type={"text"} onSubmit={() => sendMessage()} placeholder="Message..." className="bg-transparent outline-none hover:outline-none grow" />
                                          <IoMdSend
                                                size={20}
                                                onClick={() => {
                                                      sendMessage();
